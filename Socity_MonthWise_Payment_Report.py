...
class MonthWisePaymentReport:
    ...
    def load_report(self):
        ...
        df["TotalPayment"] = df[months].sum(axis=1)
    ...
    def show_table(self, data):
        ...
        for m in months:
            totals.append(round(data[m].sum(), 2))

        totals.append(round(data["TotalMaintenance"].sum(), 2))
        totals.append(round(data["TotalPayment"].sum(), 2))
        totals.append(round(data["Pending"].sum(), 2))

        blank = [""] * len(self.tree["columns"])
        self.tree.insert("", tk.END, values=blank)

        self.tree.insert("", tk.END, values=totals, tags=("total",))
...
if __name__ == "__main__":
    ...
    tk.Button(
        root,
        text="Open Payment Report",
        command=lambda: MonthWisePaymentReport(root),
        font=("Arial", 12),
        width=22,
        bg="#2ecc71",
        fg="white"
